...
public class SmsHelper {
	...
	public static void main(final String[] args) throws TwilioRestException,
		...
		// sendMessageToIndia("This is test API");

	}

	public static void sendMessage(String number, String message)
			throws TwilioRestException, TwiMLException {
		...
		smsParams.put("From", "+14085122317"); // Replace with a valid phone
												// number in your account
		smsParams.put("Body", message);
		smsFactory.create(smsParams);

		// Make a raw HTTP request to the api... note, this is deprecated style
		final TwilioRestResponse resp = client.request("/2010-04-01/Accounts",
				"GET", null);
		if (!resp.isError()) {
			System.out.println(resp.getResponseText());
		}

		IndiaSMS.sendIndiaSMS(message);
